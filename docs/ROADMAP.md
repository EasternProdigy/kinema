...
> styling + sync) has landed; **Phase 2 (accounts & multi-user, opt-in `--accounts`) is
> shipped**; and **Phase 3 (public-hardening & ops — built-in TLS, per-IP rate limiting,
> `/healthz` + `/metrics` + structured logging, per-user stream quotas) is shipped** — see the
> §2/§3 markers and the changelog. This document remains the agreed direction; we keep
> implementing phase by phase, settling the open decisions in §6 as we go.
